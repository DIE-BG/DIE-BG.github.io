...
```matlab
        ...
        % the initial conditions previously calculated in the smoothing procedure
        % is extended wit the shocks 
        init_conditions = dboverlay( ...
            init_conditions, ...
            shocks_for_this_iteration ...
        );
```

Ahora que tenemos las condiciones iniciales y los anclajes en los choques, procedemos a simular.

```matlab
        % simulate automatically exogenize the shocks and endogenize the 
        % variables
        simRes = simulate( ...
            obj.irisModel, ...
            init_conditions, ...
...
```
>> structfun(@(x) x.Range(end), init_conditions * get(obj.irisModel, 'xlist'))
ans = 
...
![Pronósticos condicionales para `h=1`](images/objective.svg)

Realizar este proceso permite obtener no solo los pronósticos condicionales para las variables del SVAR, sino también los de las variables objetivo.

---

![Choques estructurales para la iteración `h=1`](images/shocks.svg)
