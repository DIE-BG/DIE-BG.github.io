...
- A partir de los resultados en de las simulaciones de @burman1994cross, presentados en la @tbl-results, se observa, en términos generales, que valores pequeños de $h$ tienden a subestimar el MSE obtenido por validación cruzada, mientras que valores grandes de $h$ tienden a sobreestimarlo.

---

- En el caso en que $h$ es pequeño, la muestra de validación y la de estimación están altamente correlacionadas, lo que incrementa la probabilidad de acertar en el pronóstico un paso hacia adelante, sesgando a la baja la estimación del MSE por validación cruzada un paso adelante.
...
- En el caso en que se impone un valor de $h$ muy grande, la estimación del MSE por validación cruzada un paso adelante tiende a sobrestimar el verdadero MSE.

En los ejercicios de simulación de @burman1994cross, esto ocurre debido a la reducción excesiva en el tamaño de la muestra de estimación.

En la práctica, también se identifica un riesgo de sobreestimación del MSE, ya que un bloque $h$ muy grande podría excluir información importante sobre un cambio en la dinámica conjunta de los datos.
...
Luego, comparan los valores esperados y las desviaciones estándar de ambas distribuciones con el objetivo de identificar qué valor de $h$ genera una un estimador de validación cruzada más concentrado alrededor del valor esperado del MSE real.

> Los ejercicios de simulación solamente contempla los MSE *un paso hacia adelante* u horizonte 1 de pronóstico.
...
La aleatoriedad de esta magnitud proviene de la muestra aleatoria $\mathcal{X}$, por lo que el ejercicio de simulación aproxima el valor esperada de dicha magnitud:
...
El MSE real al estimar sobre la muestra completa $\mathcal{X}$ solo puede calcularse si se cuenta con una muestra independiente $\tilde{\mathcal{X}} = \lbrace \tilde{X}_{k+1}, \tilde{X}_k, \dots, \tilde{X}_1 \rbrace$, distinta a la utilizada para la estimación.
...
1. Simulamos 10,000 realizaciones de la muestra aleatoria $\mathcal{X}$, indexada por $m$ y denotada como $\lbrace x_1^{(m)}, \dots, x_N^{(m)} \rbrace$.

2. Simulamos 10,000 realizaciones de la muestra aleatoria $\tilde{\mathcal{X}}$, indexada por $w$ y denotada como $\lbrace \tilde{x}_1^{(w)}, \dots, \tilde{x}_N^{(w)} \rbrace$.
