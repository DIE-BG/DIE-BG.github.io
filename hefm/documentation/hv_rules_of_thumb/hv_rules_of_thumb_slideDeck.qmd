...
4. Se estima el valor esperado del MSE real, $E\left[PE\right]$.
...
## Propuesta de ejercicio de simulación {.appendix #sec-sim_proposal}
...
E \left[ PE(j) \right] = E \left[ 
    ...
    \left( 
        z \mapsto z^2,
        \tilde{y}_{j} - \sum_{i=1}^j \hat{A}_1^{i-1} \hat{C} - \hat{A}_1^j \tilde{y}_0
    \right)
\right]
$$
...
Esta magnitud se computaría $h \times v$ veces, una por cada combinación de $h$ y $j$.
...
Finalmente, se comparan $\hat{E}\left[PE(j)\right]$, $\hat{E}\left[CV_{HEFM}(h,v,j)\right]$ y sus respectivas distribuciones simuladas con el fin de:
...
## Función `map` {.appendix #sec-map}

> Sea $f: \mathbb{R} \to \mathbb{R}$ una función real, y sea $\mathbf{x} = (x_1, x_2, \dots, x_n) \in \mathbb{R}^n$. La operación `map(f, x)` se define como:
> $$
> \text{map}(f, \mathbf{x}) = \left(f(x_1), f(x_2), \dots, f(x_n)\right)
> $$
> Es decir, `map` aplica la función $f$ a cada componente del vector $\mathbf{x}$, y devuelve un nuevo vector en $\mathbb{R}^n$ cuyos elementos son las imágenes de los elementos originales bajo $f$.
