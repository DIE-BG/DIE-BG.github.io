...
con una estimación
diferente a la utilizada en el SVAR50-4B.
...
ambos modelos (SVAR53 y SVAR50-4B) son que, 
...
$$ 
\hat{\gamma}=\left[R^{\prime}\left(ZZ^{\prime}\otimes\Sigma_{u}^{-1}\right)R+\lambda Q^{\prime}Q\right]^{-1}R^{\prime}\left(Z\otimes\Sigma_{u}^{-1}\right)\mathbf{z}.
$$ {#eq-ridge-penalty}
...
## Matriz de restricción del SVAR50-4B
El modelo SVAR50-4B se estima con GLS y la matriz 
...
objetivo y el promedio correspondientes al SVAR50-4B,
...
:RMSE de la inflación y la tasa de interés en el SVAR50-4B
...
@fig-RMSEByVariable se ve que el SVAR50-4B sigue teniendo 
...
compararse con el RMSE del SVAR50-4B y comprobar que el error
del SVAR50-4B sigue estando mejor que el del SVAR53 incluso, con un $\lambda$
calibrado.

Para el promedio de las variables, el SVAR50-4B tiene un RMSE menor
